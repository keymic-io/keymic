...
- Agent tools follow Claude-Code-compatible naming (`Read`/`Write`/`Edit`/`Bash`/...) and exchange JSON `Data` as input + string output, allowing `[any Tool]` arrays without associatedtype gymnastics. The `Tool` protocol lives in `Sources/KeyMic/Tools/Protocol/`; concrete tools live in sibling directories (`Tools/Bash/` and `Tools/File/` today; `Tools/Search/`, `Tools/MCP/`, `Tools/Skill/`, `Tools/Agent/` in future plans). File tools share a `FileSystemActor` (in `Tools/File/`) that enforces a symlink-tolerant path-safe sandbox against `ToolContext.workingDirectory` and dispatches blocking IO off the cooperative pool.
