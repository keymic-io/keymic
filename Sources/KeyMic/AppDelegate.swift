...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func rebuildPersonasMenu() {
        ...
        for persona in PersonaStore.shared.personas {
            let isDefaultPersona = persona.id == PersonaStore.shared.activePersonaId
            let title = isDefaultPersona ? "Clear Default Persona: \(persona.name)" : "Set Default Persona: \(persona.name)"
            let item = NSMenuItem(title: title, action: #selector(selectPersona(_:)), keyEquivalent: "")
            item.target = self
