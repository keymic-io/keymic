...
struct ContextResolverTestRunner {
    ...
    static func run() async {
        await testNoSourcesProducesEmptyContext()
        await testSelectionOnly()
        await testClipboardTopOnly()
        await testClipboardHistoryOnly()
        await testSelectionAndClipboard()
        await testWindowOCRStubReturnsNil()
        await testOCRStatusUpdate()
        await testOCRErrorSwallowed()
    }

    // MARK: - Tests against PersonaContextBuilder.gather

    static func testNoSourcesProducesEmptyContext() async {
        let providers = makeProviders(selection: "SEL", clipboardTop: "CLIP", history: ["H0", "H1"], ocr: "OCR")
        let ctx = await PersonaContextBuilder.gather(sources: [], providers: providers, onStatusUpdate: { _ in })
    ...
    static func testSelectionOnly() async {
        let providers = makeProviders(selection: "SEL", clipboardTop: nil, history: [], ocr: nil)
        let ctx = await PersonaContextBuilder.gather(sources: [.selection], providers: providers, onStatusUpdate: { _ in })
    ...
    static func testClipboardTopOnly() async {
        let providers = makeProviders(selection: nil, clipboardTop: "CLIP", history: [], ocr: nil)
        let ctx = await PersonaContextBuilder.gather(sources: [.clipboardTop], providers: providers, onStatusUpdate: { _ in })
        expect(ctx.clipboardTop == "CLIP", "clipboardTop source → clipboard captured")
        expect(ctx.selection == nil, "clipboard-only → no selection")
    }

    static func testClipboardHistoryOnly() async {
        let providers = makeProviders(selection: nil, clipboardTop: nil, history: ["a", "b"], ocr: nil)
        let ctx = await PersonaContextBuilder.gather(sources: [.clipboardHistory], providers: providers, onStatusUpdate: { _ in })
    ...
    static func testSelectionAndClipboard() async {
        ...
        let ctx = await PersonaContextBuilder.gather(sources: sources, providers: providers, onStatusUpdate: { _ in })
        expect(ctx.selection == "S", "both → selection captured")
        expect(ctx.clipboardTop == "C", "both → clipboard captured")
    }

    static func testWindowOCRStubReturnsNil() async {
        let providers = makeProviders(selection: nil, clipboardTop: nil, history: [], ocr: nil)
        let ctx = await PersonaContextBuilder.gather(sources: [.windowOCR], providers: providers, onStatusUpdate: { _ in })
        expect(ctx.windowOCR == nil, "nil OCR stub → nil OCR")
    }

    static func testOCRStatusUpdate() async {
        let providers = makeProviders(selection: nil, clipboardTop: nil, history: [], ocr: "screen text")
        var statuses: [String] = []
        let ctx = await PersonaContextBuilder.gather(sources: [.windowOCR], providers: providers) { s in
            statuses.append(s)
        }
    ...
    static func testOCRErrorSwallowed() async {
