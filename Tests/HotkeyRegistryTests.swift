...
struct HotkeyRegistryTestRunner {
    static func main() {
        ...
        expect(r.conflicts(for: altQ, excluding: nil).isEmpty, "empty registry has no conflicts")

        // register clipboard panel hotkey
        r.register(altV, owner: .clipboardPanel, purpose: "Open clipboard panel")
        let c = r.conflicts(for: altV, excluding: nil)
        ...
        expect(r.conflicts(for: altQ, excluding: nil).isEmpty, "alt+q free after unregister")
        ...
        let c2 = r.conflicts(for: altQ, excluding: nil)
